...
<form name="myFrom" id="myFrom" method="post" action="">
    ...
    <div class="contanier page">
            ...
            <div class="ihuigo">
                ...
                <div class="sub_con_top">
                    ...
                    <c:forEach items="${addressList}" var="address">
                    	 <div class="contanct_c check" name="item_member_addr" id="item_member_addr_0${address.address_id}">
                       	 <label>
                            <input type="radio" name="member_addr" value="0" checked="checked" onClick="new_member_addr('${address.address_id}')" />
                            <span>${address.user_name}&nbsp;&nbsp;${address.user_tel}&nbsp;&nbsp;${address.contact_address}</span>
                            <input type="hidden" value="${address.address_id}"></label>
                    	</div>
                    </c:forEach>
                    ...
                    <!-- 新地址start-->
                    <%-- <input type="hidden" value="${jsonStr}"> --%>
      ...
      您可选择<a href="" style="background:none; padding:0px; color:#03F; font-weight:bold;" target="_blank">参与中国人民财产保险投保计划</a></span>
            &nbsp;&nbsp;
            <label><a href="" target="_blank"  class="on">我已查看并同意回收协议</a></label>
    ...
    <div class="dingdan">
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="table1">
            ...
            <tr>
	                ...
	                <td align="center" style="line-height:22px;">
	                    <h3>${mapSingle.key.shopping_price}</h3>
	                </td>
        ...
        <input type="hidden" id="hidden_map" value=""/>
        <div class="foot_bor"> <em id="sumCount">1</em>件产品总计回收价格<em id="sumPrice" style="font-size:18px;">￥2800</em> <!--<span>赠送环保券 <i>30</i>元--></span></div>
    </div>
    <!--订单-->
    </div>
    <!--提交订单-->
...
<script language="javascript">
    ...
    /*提交回购单*/
	    ...
	    var mapJson1 = ${mapJson};
	    var mapJson = mapJson1.toString();
	    $("#hidden_map").val(mapJson);
	    $("#sumCount").text(count);
	    $("#sumPrice").text(sumPrice);
	    
    function checkfrom(){
        var user_name = $("#link_name").val();
        var mobile = $("#mobile").val();
        var realName = $("#alipay_realname").val();
        var userNum = $("#alipay_username").val();
        var inputArray = $("input[name='member_addr']");
        var addressId = null;
        for(var i=0;i<inputArray.length-1;i++){
			if(inputArray[i].checked==true){
				var lab = inputArray[i].parentNode;
				var last = lab.lastChild;
				addressId = last.value;
			}
         }
        if(addressId==null){
	        ...
	        if(user_name =="" || mobile=="" || province=="" || city=="" || district=="" || realName=="" || userNum==""){
				alert("您的信息未填写完整");
				return;
	        }
        }else{
        ...
        window.location.href="<%=basePath%>/insertOrder.do?userName="+user_name+"&userMobile="
        +mobile+"&userAddress="+address+"&realName="+realName+"&userNum="+userNum+"&sumPrice="+sumPrice+"&address_detail="+address_detail+"&addressId="+addressId;
    }
</script>
</body>
</html>